...
/** Builds small JavaScript actions that operate the real AI website behind AIHub's native UI. */
public final class DomScripts {
    private DomScripts() {}

    public static String send(AiProvider provider, String text) {
        return """
            (()=>{
              ...
              const score=e=>{if(!usable(e))return-1;const r=e.getBoundingClientRect();let n=0;if((e.tagName||'').toLowerCase()==='textarea')n+=1000;if(e.isContentEditable)n+=900;if(e.getAttribute('role')==='textbox')n+=800;if(r.top>innerHeight*.45)n+=500;return n;};
              const candidates=[...document.querySelectorAll('textarea,[contenteditable="true"],[role="textbox"]')].filter(usable).sort((a,b)=>score(b)-score(a));
              const input=pick(inputSelectors)||candidates[0];
              if(!input)return JSON.stringify({ok:false,stage:'input'});
              input.focus();
              if('value' in input){let p=input;let setter=null;while(p&&!setter){const d=Object.getOwnPropertyDescriptor(p,'value');if(d&&d.set)setter=d.set;p=Object.getPrototypeOf(p);}if(setter)setter.call(input,value);else input.value=value;}else{input.textContent=value;}
              input.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:value}));
              input.dispatchEvent(new Event('change',{bubbles:true}));
              const words=['send','submit','发送','提交','ask'];
              const semantic=[...document.querySelectorAll('button,[role="button"]')].filter(usable).find(e=>{const t=[e.getAttribute('aria-label'),e.getAttribute('title'),e.textContent].filter(Boolean).join(' ').toLowerCase();return words.some(w=>t.includes(w));});
              const button=pick(sendSelectors)||semantic;
              if(button){button.click();return JSON.stringify({ok:true,method:'button'});}
              input.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true}));
              input.dispatchEvent(new KeyboardEvent('keyup',{key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true}));
              return JSON.stringify({ok:true,method:'enter'});
            })()
            """.formatted(jsString(text), jsArray(provider.inputSelectors()), jsArray(provider.sendSelectors()));
    }

    public static String newChat(AiProvider provider) {
        return click(provider.newChatSelectors(), List.of("new chat", "new conversation", "新对话", "新聊天"));
    }

    public static String stop(AiProvider provider) {
        return click(provider.stopSelectors(), List.of("stop generating", "stop", "停止生成", "停止"));
    }

    public static String setAppMode(AiProvider provider, boolean enabled) {
        return """
            (()=>{
              ...
              if(window.__aihubObserver)window.__aihubObserver.disconnect();
              const schedule=()=>{if(window.__aihubTimer)clearTimeout(window.__aihubTimer);window.__aihubTimer=setTimeout(apply,150);};
              window.__aihubObserver=new MutationObserver(schedule);window.__aihubObserver.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class','style','hidden','aria-hidden']});
              return apply()?'on':'waiting';
            })()
    ...
    private static String jsArray(List<String> values) {
        return "[" + values.stream().map(DomScripts::jsString).reduce((a, b) -> a + "," + b).orElse("") + "]";
    }
